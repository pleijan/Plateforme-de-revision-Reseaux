...
Et donc parmis toutes ces tâches à accomplir seulement la commande reseau tcpdump n'a pas été accomplie.
...
Tout au long du projet, nous nous sommes confronter à certains problèmes. La plus part des fonctionnalités qui sont programmés, nécessitaient des cours de M.Hoguin. C'est pourquoi nous devions bien comprendre les notions avant de les programmés. Nous nous sommes donc entrainés sur papier avant de le programmer. Par exemple, pour les divisions de réseaux en sous-réseaux avec les notations CIDR et VLSM. Mais aussi pour les calculs de CRC polynomial. Nous prenions des rendez-vous chaque semaine pendant les TP des mercredis, pour poser nos questions à notre client M.Hoguin, qui nous expliquait les notions non comprises. Il nous conseillait aussi sur l'aspect graphique du site. Celui-ci a changé de nombreuse fois. Nous avons appris a utilisé Bootstrap une librairie qui permet d'embellir les pages internet.

Pour les fonctionnalités concernant les adresses IP nous avions prévus de mettre un champ pour chaque partie de l'adresse IP. Nous trouvions cela plus clair pour les étudiants, pour différentier les différentes partie de l'adresse IP. Cependant, il s'est avérer qu'en faisant plusieurs tests pour vérifier les résultats, que le fait de cliquer sur chaque champs pouvait ennuyer l'utilisateur. Ex : pour une adresse IP de type : 192.168.2.3 il y avait quatres champs, un pour 198, un pour 168 etc...
C'est pourquoi nous avons supprimer tout ces champs et avons laissé un seul champ pour l'adresse IP. C'était plus rapide, et c'était quand même clair. 

Après avoir programmé la fonctionnalité de division de réseaux en sous réseaux avec la notation CIDR ainsi que les deux type de CRC, il n'y avait plus d'erreurs. Mais en faisant tester ces applications à M.Hoguin, nous nous sommes rendus compte que quelques incohérences apparaissaient pour certains sous-réseaux et pour les résultats des CRC. Et à chaque fois, que l'on corrigeait un bug un autre apparaissait. Au final, avec de la persevérence nous avons résolus tout ces problèmes. Ils ont été tester de nombreuse fois et ont passé les tests avec succès.
...
Aussi lors de l'appuie sur un bouton après rechargement de la page, les champs étaient réinitialiser. Et on pouvait, ne plus se souvenir des informations que l'on avait inscrit dans les champs. Nous avons tenté une première approche qui est d'afficher les informations entrés par l'utilisateur. Mais nous nous sommes dit, que ce serait mieux qu'après le chargement de la pages les champs ne se réinitialisent pas. On a donc donner une valeur par défaut à chaque champs qui est l'information entré par l'utilisateur et envoyé par la méthode GET. on utilise la méthode isset() pour savoir si elle est vide ou non. Finalement, ce problème a été résolu.

Concernant la fonctionnalité d'exécution de commandes réseaux. Nous devions pensé à un moyen de récupérer les informations qui apparaissent sur la console. C'est pourquoi après plusieurs recherche avec M.Hoguin, nous avons utilisé la commande de redirection de sortie ">" pour créer un fichier qui contient ces informations. Il suffisait ensuite de lire ce fichier avec PHP. Ces commandes fonctionnent seulement lorsque le site est d'héberger sur un serveur de l'iut. Certaines commandes ne fonctionnaient pas car fallait utiliser la commande superutilisateur sudo. Nous l'avons donc installer et cela à fonctionner.
...
Conclusion: Au cours de ce projet, nous avons pu apprendre/réviser les notions de s2 du modules de réseaux. Cela à permis aussi de consolider nos connaissances en PHP. De découvrir un peu plus le Javascript et le Bootstrap. Aussi, cela nous a permis de renforcer la méthodologie de travail, c'est-à-dire d'abord penser le projet puis le programmer. Et enfin, la situation sanitaire nous a fait acquérir de l'expérience. Nous avons appris à quand même mener à bien un projet même s'il y a des abandons et que le travail devient plus lourd pour les autres membres de l'équipe.
