...
Pour nous c'etait impensable que cette application ne soit pas facile à utiliser. Premierement, car elle sera utilisée principalement par des étudiants. Aussi, parce qu'elle servira de support de cours pour ces étudiants. Il a fallut se mettre à la place des étudiant pour rendre cette plateforme simple d'utilisation. Et, il doit être beau à voir pour qu'ils aient envie de l'utiliser. On a commencé par la page d'accueil en mettant des grandes images avec le nom de chaque fonctionnalité, comme vous pouvez voir Annexe 2.1 De plus, via le menu en haut de chaque page, il est très facile de voyager entre chaque fonctionnalité. Celles-ci sont composées de cours expliquant la méthode à utiliser si l'on veut le faire soi-même, et une application pour mettre en pratique ce cours. Pour rendre ces applications plus simple, une indication est présente à chaque fois qu'une information est attendu. Et, en cas de problème des messages d'erreurs apparaîtront et vous expliqueront ce qu'il ne va pas cf Annexe 2.2. Enfin vous pouvez voir à quoi ressemble l'affichage d'un résultat d'une application (exemple affichage du VLSM) en Annexe 2.3. 

#### La performance :

Aussi étant héberger en ligne, il n'est pas nécessaire d'avoir une machine surpuissante pour utiliser cette platforme. Et les résultats apparaissent rapidement, car ce ne sont pas des algorithmes très complexes. En programmant, nous avons tout de même optimiser le code, pour qu'il puisse afficher au mieux les résultats. Et donc, cette platforme est accessible, depuis toutes les machines de l'iut, et même depuis chez-soi en utilisant le logiciel Putty.
...
La méthode SCRUM est une méthode agile, où le logiciel est développé en évoluant sous formes de sprints. Ce sont des phases de développement qui sont concentrés sur des fonctionnalités. L'équipe est donc autoorganisée et peut choisir la meilleure façon d’accomplir sa tâche. De plus, cette méthode inclut la participation et l’implication active du client durant l’intégralité du projet.
...
#### un agenda hybride :
Pour produire ce projet nous nous sommes répartie les taches sur le temps mais globalement sur une semaine type, le soir en semaine nous travaillons sur nos fonctionnalités et le week-end nous verifions si tout ce qui a été codé est bien fonctionnel.
...
Aussi nous avons utilisé la plateforme Zoom pour communiquer avec notre client, faire une réunion avec lui pour faire un point sur la situation, lui poser des questions, savoir ce qu’il pense de notre avancé du projet. Aussi, lors de nos TP de programmations Web en présence les mercredis, nous faisions un points sur le projet toutes les semaines avec M.Hoguin et les membres de l'équipe.

### Planification détaillée du projet
#### Ordonnancement :

Pour ce projet nous avons essayer d'appliquer une une méthode de travail simple, nous voulions au debut commencer par le recueil des besoins ainsi que la conception, commencer à coder et écrire en même temps le rapport. Au final nous avons changer de méthode de travail, nous avons pratiquer une sorte de méthode scrum, car nous essayons de travailler le plus possible sur le projet tout en travaillant en parallèle pour l'université. 

#### Diagramme de gantt :

Nous avons fait un diagramme de Gantt pour pouvoir situer les différentes tâches du projet dans le temps, comme on peut le voir sur le diagramme mis en Annexe 3.1. Nos travaux étaient répartis sur plusieurs semaines. Nous avons d’abord réfléchi sur le projet avec tout le monde pendant une semaine. Ensuite nous avons réécrit le cahier des charges qui nous a pris environ deux semaines. Puis, nous avons fait en parallèle le développement et le rapport jusqu’au 18 décembre, date où le premier rapport devait être rendu. A cette date, les fonctionnalités de Traductions d'adresses IP, et de division de sous-réseaux avec la notation VLSM ont été programmer. Aussi une premiere page d'accueil et style du site à été créer. Enfin, nous avons continuer a developpé le reste des fonctionnalités, tout en améliorant l'aspect du site en général. De plus, on a continué d'écrire ce rapport jusqu'à la date finale de rendu le 07/04/2021. Nous travaillions toutes les semaines lorsque l'on finissait plutôt après les cours de l'université et aussi le week-end.

#### La gestion des compétences :

Raffaele avais plus de connaissances et d'experiences sur le coté graphique d'un site internet et sur les divisions de réseaux tandis que Tom avait plus de faciliter pour construire des algorythmes notamment pour le CRC ou les traductions d'adresses IP.

#### La gestion des risques :


Tout d'abord voici la liste des risques qui pourraient arriver pendant notre projet :
...
Pour determiner la probabilité des risques et leurs impacts on va créer des echelles définie ci-dessous :

On va créer une échelle pour la probabilité de réalisation d’un risque qui va de 1 à 5, défini dans le tableau disponible en annexe 3.2

On va créer une deuxième échelle pour l’impact sur le projet allant de 1 à 3, défini dans le tableau disponible en annexe 3.3

Et donc grâce aux differentes échelles on peux déterminer un tableau avec les risques et leurs probabilités d'existences ainsi que leurs impacts qui est affiché dans l'annexe 3.4.

En étudiants les risques qui pourraient arriver, nous pouvons mieux nous préparer à l'avance. C'est pourquoi, il est important de le faire.
...
Les étudiants pourront utiliser ces fonctionnalités. Vous pourrez trouver le schéma des cas d'utilisations en Annexe 3.5.
...
- 5 commandes réseaux sous linux (ping, nmap, nslookup, man et tcpdump).
- un glossaire qui repertorie l'ensemble des mots important.
- et un manuel d'utilisation.

Et donc parmi toutes ces tâches à accomplir seulement la commande reseau tcpdump n'a pas été accomplie.

### Des evolutions possibles ?

Ce projet est-il évolutif ? La réponse est oui, tout d'abord car les cours qui y sont présent sont ceux de premiere années on pourra rajouter les cours du semestre 3 et ensuite la spécialisation réseaux du semestre 4. On peux aussi l'étendre à n'importe quel module, ce qui offre une possibilité d'extensions tout à fait énorme.

### Des acquis divers et variés
#### Savoir-être

Ce projet nous a permis d'apprendre à travailler en équipe, de faire face à des réduction d'effectifs, des crises sanitaires, tout en respectant les délais pour produire notre projet. Nous nous sommes entraidés, motivés, et avons essayé de tenir bons ensemble pour dépasser les difficiles moment de la situation sanitaire. Et, grâce à cet experience, c'est évident que nos capacités en sont sortie grandie.

#### Savoir-Faire

Nous avons aussi acquis de l'expérience niveau technique, car en effet, de nombreux obstacles on fait surface qui nous ont forcer à nous documenter, à essayer maintes et maintes fois. Ainsi, pour résoudre ces problemes, nous nous sommes donc servis à la fois de tout nos cours de programmation web, de mathematiques, mais aussi de certains site comme la documentations de php (php.net) ou encore W3School qui repertorie de nombreux sujet pour aider tout les developpeurs. Aussi nous avons dû approfondir nos connaissances en Javascript pour la fonctionnalité de division de réseau avec la notation VLSM. Nous avions dû apprendre à utiliser la librairie Bootstrap pour embellir les pages.

### Une Experience fondamentale
## Problemes rencontrés

Tout au long du projet, nous nous sommes confronter à certains problèmes. La plupart des fonctionnalités qui sont programmés, nécessitaient des cours de M.Hoguin. C'est pourquoi nous devions bien comprendre les notions avant de les programmés. Nous nous sommes donc entrainés sur papier avant de les programmés. Par exemple, pour les divisions de réseaux en sous-réseaux avec les notations CIDR et VLSM. Mais aussi pour les calculs de CRC polynomial. Nous prenions rendez-vous chaque semaine pendant les TP des mercredis, pour poser nos questions à notre client M.Hoguin, qui nous expliquait les notions non comprises. Il nous conseillait aussi sur l'aspect graphique du site. Celui-ci a changé de nombreuses fois. Nous avons appris a utilisé Bootstrap une librairie qui permet d'embellir les pages internet.

Pour les fonctionnalités concernant les adresses IP nous avions prévus de mettre un champ pour chaque partie de l'adresse IP. Nous trouvions que cela allait être plus clair pour les étudiants, pour différentier les différentes parties de l'adresse IP. Cependant, il s'est avéré qu'en faisant plusieurs tests pour vérifier les résultats, le fait de cliquer sur chaque champs pouvait ennuyer l'utilisateur. Ex : pour une adresse IP de type : 192.168.2.3 il y avait quatres champs, un pour 198, un pour 168 etc...
C'est pourquoi nous avons supprimer tous ces champs et avons laissé un seul champ pour l'adresse IP. C'était plus rapide, et c'était quand même clair. 

Après avoir programmé la fonctionnalité de division de réseaux en sous réseaux avec la notation CIDR ainsi que les deux type de CRC, il n'y avait plus d'erreurs. Mais en faisant tester ces applications à M.Hoguin, nous nous sommes rendus compte que quelques incohérences apparaissaient pour certains sous-réseaux et pour les résultats des CRC. Et à chaque fois, que l'on corrigeait un bug un autre apparaissait. Au final, avec de la persevérence nous avons résolus tout ces problèmes. Ils ont été tester de nombreuses fois et ont passé les tests avec succès.

Pour la fonctionnalité de division de sous-réseaux avec la notation VLSM, il fallait indiquer le nombre d'hôtes que l'on voulait pour chaque sous-réseaux. C'est pourquoi, nous avions pensé à un bouton ajout qui ajoutait un champ pour inscrire le nombre d'hôtes. Un petit problème qui peut sembler sans importance mais qui est très ennuyeux lorsque l'on utilise le logiciel de façon prolongé est qu'à chaque appui du bouton ajout la page se rechargeait. Pour pallier à ce problème, nous avons donc dû appronfondir nos connaissances en Javascript qui permettait ainsi d'ajouter des champs sans actualiser la page. Au début, du semestre 3, nous avions appris le langage PHP. Donc, il nous semblait logique d'utiliser celui-ci dès le début. Mais effectivement celui effectue une actualisation de la page lors de l'appuie d'un bouton.

Aussi lors de l'appuie sur un bouton après rechargement de la page, les champs se réinitialisaient. Et on pouvait, ne plus se souvenir des informations que l'on avait inscrit dans les champs. Nous avons tenté une première approche qui est d'afficher les informations entrés par l'utilisateur. Mais, nous nous sommes dit, que ce serait mieux qu'après le chargement de la page les champs ne se réinitialisent pas. On a donc donné une valeur par défaut à chaque champs qui est l'information entré par l'utilisateur et envoyé par la méthode GET. on utilise la méthode isset() pour savoir si elle est vide ou non. Finalement, ce problème a été résolu.

Et concernant les divisions de sous-réseaux, il fallait indiquer le masque. Au début, nous avions fait le choix de faire en sorte que l'utilisateur entre le masque avec le slash (la notation CIDR). Mais en testant, nous nous sommes rendus compte que parfois écrire le masque en entier peut être rapide, si l'on a pas la notation CIDR sous la main. Car avant, si l'on avait le masque en entier, nous devions compter à la main le nombre de un du masque de réseaux convertit en binaire. C'est pourquoi nous avons fait en sorte que l'utilisateur puisse entrer le masque en entier ou bien sa version réduite.

Concernant la fonctionnalité d'exécution de commandes réseaux. Nous devions pensé à un moyen de récupérer les informations qui apparaissent sur la console. C'est pourquoi après plusieurs recherche avec M.Hoguin, nous avons utilisé la commande exec qui permet d'exécuter ces commandes. Celle-ci fonctionnent seulement lorsque le site est d'héberger sur un serveur de l'iut. Certaines commandes ne fonctionnaient pas car fallait utiliser la commande superutilisateur sudo. Nous l'avons donc installer et cela à fonctionner.

En outre, nous avons rencontrer des problèmes avec la situation sanitaires. Il est difficile de se voir, nous avons du réaliser ce projet à distances. Puis, certains membre de notre équipe ont abandonnés, nous avons du réorganiser les tâches et faire face à ce problème.

Conclusion: Au cours de ce projet, nous avons pu apprendre/réviser les notions de semestre deux du modules de réseaux. Cela à permis aussi de consolider nos connaissances en PHP. De découvrir un peu plus le Javascript et le Bootstrap. Aussi, cela nous a permis de renforcer la méthodologie de travail, c'est-à-dire d'abord penser le projet puis le programmer. Et enfin, la situation sanitaire nous a fait acquérir de l'expérience. En effet, nous avons appris à quand même mener à bien un projet même s'il y a des abandons et que le travail devient plus lourd pour les autres membres de l'équipe.
...
### Enoncé
L’application web devra permettre aux étudiants débutants d’aborder les notions de réseau vues au semestre un sous un angle pédagogique et ludique au travers de possibles animations. L'application WEB classique sera hébergée en intranet et devra contenir divers fonctionnalités accessible depuis un menu. Le menu devra contenir le logo de l'UVSQ et devra contenir toutes les fonctionnalités présent sur le site comme :
...
- avoir un glossaire qui pour les etudiant recenserons les mots clé du module réseau et pour l'administrateur pourra gérer ces mots clés pour en ajouter ou supprimer.
...
`r if(knitr:::pandoc_to() == "latex") {paste("\\newpage")}`
## Annexe 2 : le livrable
## Annexe 2.1 : capture d'écran de l'accueil du site 
...
![La page d'accueil du site](../Manuel_Utilisateur/Image_utilisée/index.png)
figure : La page d'accueil du site
## Annexe 2.2 : capture d'écran de message d'erreur aidant les étudiants
...
| Responsable de la Redaction: Tom                                                     |                    |   |   |   |
|                                                                                           |                    |   |   |   |
![Exemple d'une erreur qui guidera les étudiants](../Manuel_Utilisateur/Image_utilisée/erreurCRC.png)
figure : Exemple d'une erreur qui guidera les étudiants
## Annexe 2.2 : capture d'écran d'affichage d'un résultat (division sous-réseaux VLSM)

| Exemple d'affichage d'un résultat (division sous-réseaux VLSM) | Version : 1        |   |   |   |
|-------------------------------------------------------------------------------------------|--------------------|---|---|---|
| Document : Capture d'écran                                                              | Date: <28/03/2021> |   |   |   |
| Responsable de la Redaction: Raffaele                                                     |                    |   |   |   |
|                                                                                           |                    |   |   |   |
![Exemple d'affichage d'un résultat](../../img/exemple_resultat.PNG)
figure : Exemple d'affichage d'un résultat pour la division sous-réseaux VLSM
## Annexe 3: Analyse des besoins

## Annexe 3.1: diagramme de gantt represantant l'avancement du projet
...
|                                                                                           |                    |   |   |   |
![](../projet_tuto.png)
figure : diagramme de gantt represantant l'avancement du projet
## Annexe 3.2 : tableau de l'echelle de probabilitée du risque 
...
`r if(knitr:::pandoc_to() == "latex") {paste("\\newpage")}`
## Annexe 3.3 : tableau de l'echelle de l'impact d'un risque sur le projet
...
1 Impact faible, 3 Impact grand

`r if(knitr:::pandoc_to() == "latex") {paste("\\newpage")}`
## Annexe 3.4: tableau des differents risques avec leurs impacts
...
Tableau des differents risques avec leurs impacts

## Annexe 3.5: les cas d'utilisations
...
![figure : cas d'utilisations](../img/casUtilisationFin.jpg)
...
Nous allons maintenant détailler les cas d'utilisations au **niveau utilisateur**
